...
BIOMEDICINE_SCENARIO_SPECS: dict[str, BiomedicineScenarioSpec] = {
    ...
    "docking_match": BiomedicineScenarioSpec(
        ...
        "scan-search",
        "cyan",
        (("reference_pose", "共晶参考"), ("strict_geometry", "几何严格")),
        (
            _range("match_weight", "匹配权重", 0.2, 1.0, 0.05),
            _range("collision_penalty", "碰撞罚项", 1.0, 4.0, 0.1),
        ),
        {"match_weight": 0.65, "collision_penalty": 2.4},
        "hybrid",
        "problem",
    ...
    "active_center": BiomedicineScenarioSpec(
        ...
        "orbit",
        "amber",
        (("antiferromagnetic", "反铁磁耦合"), ("ligand_field", "配体场扰动")),
        (
            _range("exchange_coupling", "交换耦合 J", -2.0, 2.0, 0.1, " meV"),
            _range("local_field", "局域场 h", -1.0, 1.0, 0.05, " meV"),
        ),
        ...
        "spin_correlation",
        "active-center",
        ("structure_preview", "effective_model_design"),
        "preview",
        _VQE_PROFILE,
    ),
