...
当前开发分支包含 16 组生物医药 fixture：9 组小分子电子结构、2 组公开结构派生的离散构象匹配数据、3 组项目生成的有效自旋模型、2 组项目生成的小肽离散构象库。新增高级 fixture 在 V2 发布验收完成前仍属于开发资产，不表示高级入口已经正式发布。
...
十六组 manifest 均通过统一运行时契约校验，明确记录原始输入 checksum 状态、生成工具版本和参数、单位、坐标系、变量顺序、经典参考方法与软件版本、标准预设参考结果、允许说法和限制。项目生成且没有外部原始文件的数据以 `raw_file_sha256: null` 明确记录，不用缺失字段掩盖来源边界。
