...
顶部状态栏只保留服务在线、执行可审计和语言切换。左侧场景导航底部的数据来源统一显示为“合成数据 / Synthetic Data”，输入区统一显示“参数预设 / Parameter Preset”。

## 验证

- 组件测试确认量子实验不存在通用门切换和 SVG，只显示 QAOA 逻辑层。
- 组件测试确认执行上下文只有 Mode、Seed、Shots 和 Wall time。
- 组件测试确认顶部状态栏不显示数据来源状态项，参数区使用中性的预设名称。
- 主界面源码和最终静态资源不再包含相关旧称。
- React 20 项测试、Python 45 项定向回归和 Ruff 检查通过，TypeScript 检查及生产构建通过。
