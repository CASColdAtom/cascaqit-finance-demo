...
打开 <http://127.0.0.1:8000>。选择场景和预设，调整业务参数后运行。工作台会加载按场景验收的 shots、算法、层数和参数搜索配置，修改任一执行参数后标记为自定义配置。Digital QAOA 支持 `p=1~3`，Hybrid QAOA 支持 `p=1~2`；两者都可以使用固定层数或自动选层。Analog QAA 使用固定的一层语义。执行来自本地模拟器，不访问网络。
