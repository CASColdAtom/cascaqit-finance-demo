...
金融领域保留七个成熟场景。生物医药领域提供四个独立场景入口：小分子电子结构和金属活性中心已接通 CASCAQit Digital VQE、QWC 有限 shots 测量与精确对照，构象匹配已接通 QUBO、Hybrid D-A-D QAOA 和 Digital QAOA；小肽能景当前仍为结构预览，不生成虚构的量子结果。
...
打开 <http://127.0.0.1:8000>，先在顶部切换金融或生物医药领域，再选择场景。金融场景继续使用原有推荐执行配置；生物医药 Pauli 场景使用各自独立的 VQE 配置。执行全部来自本地模拟器，不访问网络。
...
- 生物医药当前有 H2 电子结构、1HSG 构象匹配和金属活性中心三条真实本地模拟执行链；小肽能景仍是明确标记的结构预览。
...
浏览器验收脚本覆盖 `1440 x 900`、`1280 x 720` 和 `390 x 844`，检查四个生物医药场景切换、小肽预览门禁、构象匹配、金属活性中心和 H2 运行、页面级横向溢出、结构 SVG 和量子图表 canvas 实绘像素。
